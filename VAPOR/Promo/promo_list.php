...
$options = array(CURLOPT_URL =>'http://75.156.172.79:8000/store/promotion/list?format=json',
...
foreach($variables as &$value) {
  ...
  if (strlen($value) > 1) {
    ...
    $this_disc = str_replace('"', "", $separate[1]); //remove quotes
    //handle attr[2]
    ...
    $this_end = str_replace(']',"",$this_end); //remove end bracket
    
    $select_this = "promo_select.php?id=" . $this_id;
    $delete_this = "promo_delete.php?id=" . $this_id;
    
    //Create Table Row with this Info
    
    echo "<tr>";
    echo "<td>" . $this_id . "</td>";
    echo "<td>" . $this_disc . "%</td>";
    echo "<td>" . $this_start . "</td>";
    echo "<td>" . $this_end . "</td>";
